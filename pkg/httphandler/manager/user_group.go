...
// UserGroupHandler returns an http.HandlerFunc for the user group endpoint.
func UserGroupHandler(mgr *coremanager.Manager) (string, http.HandlerFunc, *openapi.PathItem) {
	...
	return "user/{user}/group", func(w http.ResponseWriter, r *http.Request) {
			...
			Post: &openapi.Operation{
				...
				RequestBody: &openapi.RequestBody{Description: "Array of group identifiers to add to the user.", Required: true, Content: map[string]openapi.MediaType{"application/json": {Schema: userGroupListSchema()}}},
				Responses: map[string]openapi.Response{
					"200": {Description: "Updated user.", Content: map[string]openapi.MediaType{"application/json": {Schema: userSchema()}}},
					"400": {Description: "Invalid user ID, request body, or group identifiers."},
			...
			Delete: &openapi.Operation{
				...
				Parameters:  []openapi.Parameter{{Name: "user", In: openapi.ParameterInPath, Description: "User ID.", Required: true, Schema: userIDSchema}},
				RequestBody: &openapi.RequestBody{Description: "Array of group identifiers to remove from the user.", Required: true, Content: map[string]openapi.MediaType{"application/json": {Schema: userGroupListSchema()}}},
				Responses: map[string]openapi.Response{
					"200": {Description: "Updated user.", Content: map[string]openapi.MediaType{"application/json": {Schema: userSchema()}}},
					"400": {Description: "Invalid user ID, request body, or group identifiers."},
					"404": {Description: "User or group not found."},
...
func addUserGroup(ctx context.Context, mgr *coremanager.Manager, w http.ResponseWriter, r *http.Request, user schema.UserID) error {
...
func removeUserGroup(ctx context.Context, mgr *coremanager.Manager, w http.ResponseWriter, r *http.Request, user schema.UserID) error {
	var req schema.UserGroupList
	if err := httprequest.Read(r, &req); err != nil {
		return httpresponse.Error(w, httpresponse.Err(http.StatusBadRequest), err.Error())
	}
