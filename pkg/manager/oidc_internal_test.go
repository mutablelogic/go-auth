...
func Test_oidc_internal_001(t *testing.T) {
	...
	t.Run("OIDCConfigUsesConfiguredIssuer", func(t *testing.T) {
		...
		assert.Equal(oidc.AuthorizationURL("https://issuer.example.test/api"), cfg.AuthorizationEndpoint)
		...
		assert.Equal([]string{oidc.ResponseTypeCode}, cfg.ResponseTypes)
		assert.Equal([]string{"authorization_code", "refresh_token"}, cfg.GrantTypesSupported)
		assert.Equal([]string{oidc.ScopeOpenID, oidc.ScopeEmail, oidc.ScopeProfile}, cfg.ScopesSupported)
		assert.Equal([]string{oidc.CodeChallengeMethodS256}, cfg.CodeChallengeMethods)
